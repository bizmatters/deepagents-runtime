...
- ✅ Correct: `deepagents-runtime-cache.intelligence-deepagents.svc:6379`
...
- ❌ Wrong: Using `kubectl port-forward` to access services
- ✅ Correct: Run tests as Kubernetes Jobs inside the cluster

**Pitfall 4: Skipping tests when infrastructure unavailable**
- ❌ Wrong: `pytest.skip("Service not available")`
- ✅ Correct: Let tests fail with clear error messages
...
1. POST /deepagents-runtime/invoke
   ├─ Submit job with agent_definition and input_payload
   ├─ Receive thread_id and status="started"
   └─ Validate response structure

2. WebSocket /deepagents-runtime/stream/{thread_id}
   ├─ Connect to WebSocket with thread_id
   ├─ Receive streaming events: on_state_update, on_llm_stream
   ├─ Validate event format: {"event_type": "...", "data": {...}}
   ├─ Check for "files" field in final on_state_update
   └─ Receive "end" event

3. GET /deepagents-runtime/state/{thread_id}
   ├─ Query final execution state
   ├─ Validate status (completed/failed/running)
   └─ Verify generated_files if completed

4. Error Handling
   ├─ Test 404 for invalid thread_id
...
- ✅ Test one step at a time with clear validation
...
- ❌ Skip tests when infrastructure is unavailable
- ❌ Test all steps at once without validation
- ❌ Use fallback logic to hide infrastructure issues
- ❌ Make real LLM API calls in tests
- ❌ Assume localhost connectivity in cluster tests
...
1. ✅ Step 1: POST /deepagents-runtime/invoke - IMPLEMENTED
2. ⏳ Step 2: WebSocket /deepagents-runtime/stream/{thread_id} - TODO
3. ⏳ Step 3: GET /deepagents-runtime/state/{thread_id} - TODO
4. ⏳ Step 4: Error handling tests (404, 400, 500) - TODO

**Current Status:**
- Infrastructure: ✅ Running in zerotouch-preview cluster
- Services: ✅ PostgreSQL, Dragonfly, NATS all healthy
- Test Structure: ✅ Incremental step-by-step approach
- Credentials: ✅ Auto-generated and available via secrets
- Blocking Issue: ❌ Test needs to run inside cluster, not locally
...
✅ Step 1 Validation Complete - PASSED
Status: ✅ PASSED - HTTP POST /deepagents-runtime/invoke endpoint is working correctly
